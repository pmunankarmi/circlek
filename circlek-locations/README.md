...
The plugin automatically replaces that page's old table content and preserves the active theme header and footer. Version 1.4.1 replaces the previous location posts with every row from the supplied update sheets: 42 Saudi Arabia locations (13 fuel stations and 29 convenience stores) and 14 United Arab Emirates stores. Store codes are visible in the directory and searchable. It also refreshes GitHub release metadata immediately when an administrator uses WordPress's **Check again** action. Use a full-width page template if the active theme constrains page content to a narrow column.
